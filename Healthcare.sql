...
,sum(`dia1`.`Number of patients in nPCR summary`) as nPCRsummary,sum(Numberofpatientsinhypercalcemiasummary) as hypercalcemiasummary from health.dia1 ;
...
select State,count(`dia1`.`#ofDialysisStations`) as Dialysis_Stations from health.dia1 group by State order by Dialysis_Stations desc;
...
select concat(round(sum(PY2020PaymentReductionPercentage)/count(case when PY2020PaymentReductionPercentage !='No Reduction' then 1 end)* 100,2),'%') as 'Average Payment Reduction Rate' from health.dia2 ;